...
// Store the geoJSON url seperately to make the code look a little cleaner
...
let overlays = {
  ...
  "Major Earthquakes (Past Week)": majorEarthquakes
};
...
d3.json(majorEarthquakeData).then(function(data) {
  ...
  // Change the color function to use three colors for the major earthquakes based on magnitude
  function getColor(magnitude){
    if (magnitude>6){
      return '#6F0000';
    }
  ...
  // Use the function that determines the radius of the earthquake marker based on magnitude
  function getRadius(magnitude) {
    if (magnitude === 0) {
      return 1;
    }
  ...
  // Add the majorEarrthquakes Layer to the map
